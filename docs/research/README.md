...
| [انحراف VWAP وتركّز الحجم الساعي](vwap_deviation_and_volume_concentration.md) | ❌ مرفوضتان (6 من 6) + متابعتان تشخيصيتان | ثاني وثالث ميزة من بيانات ساعية حقيقية. `VWAP_DEVIATION`/high: **أقوى نتيجة بين كل الميزات الداخل-يومية** (`frac_significant=57%`) لكن `consistent_sign=False`. تشخيص إضافي: نفس نافذة انهيار FTX (٢٠٢٢-١١ إلى ٢٠٢٣-٠١) تكسر RSI وVWAP معاً — تأكيد مستقلّ لنظام سوق حقيقي. اختبار تعريف مباشر لهذا النظام على مستوى الأصل الفردي (`CRASH_REBOUND_REGIME`) أعطى نتيجة مختلطة (النظام منتشر جداً، 73% من العيّنات) — يدعم تعريفاً على مستوى السوق ككل مستقبلاً. `VOL_CONC_HHI` مرفوضة بوضوح. |

## الحالة الحالية والخطوة التالية

المرحلة ٣ من خطة المشروع مكتملة بأدواتها الثلاث. التوسّع إلى 50 أصلاً (بدل 5) هو ما أنتج أول قبول فعلي (H003). مسار `SURVEY_CANDIDATES_ROBUST` (598 مرشّحاً، 192 مؤشّراً فريداً) مُغلَق الآن بالكامل بعد خمس دفعات (154 مرشّحاً فريداً، ~450 محاولة) وترقية H003 النهائية للثلاثة المرشّحين الذين عبروا المعيار على عيّنة صغيرة — **لم ينجُ أيّ منهم**، فبقي H003 (`NATR_14`) الفرضية الوحيدة المقبولة من عائلة pandas_ta التقليدية. الجولة الأولى من [الفرضيات الأعمق](deeper_hypotheses_round1.md) (63 توليفة، 9 عائلات مبنية على أدبيات اقتصادية-إحصائية حقيقية) لم تُضِف اكتشافاً جديداً أيضاً، وثلاثية الميزات الساعية الأولى (كوفمان، [VWAP وتركّز الحجم](vwap_deviation_and_volume_concentration.md)) رُفضت رسمياً أيضاً — H003 لا يزال الاكتشاف الجوهري الوحيد. **أقرب نتيجة لعبور المعيار حتى الآن**: `VWAP_DEVIATION`/high (57% اتساق إحصائي، لكن الاتجاه غير ثابت عبر النوافذ) — تشخيص هذا التذبذب (بأسلوب تشخيص RSI في `proper_indicator_signals.md`) خطوة تالية واعدة لم تُنفَّذ بعد. البنية المعمارية (`intraday_*`) جاهزة الآن لأي ميزة داخل-يومية إضافية بلا عمل معماري جديد.
