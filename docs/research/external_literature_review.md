...
### ٨) تحديث لاحق: ثلاث زوايا رياضية من الأدبيات اختُبِرت فعلياً — كلّها مرفوضة، تؤكّد التوصية الأولى

بعد كتابة هذه المراجعة، اختُبِرت فعلياً ثلاث فئات ميزات إضافية من الأدبيات الكمّية الكلاسيكية (غائبة تماماً عن pandas_ta) على مقياس H003 الكامل (50 أصلاً/30 نافذة)، كل واحدة تختبر جانباً مختلفاً من "التقلّب" أو "الانعكاس":

| الفئة | ما تقيسه | النتيجة |
|---|---|---|
| [مقدّرات Parkinson/GK/RS/YZ/Amihud/Roll](new_volatility_liquidity_estimators.md) | حجم التقلّب (بدائل أكفأ إحصائياً لـ`NATR_14`) + سيولة | ❌ 18/18 مرفوضة |
| [أسّ هيرست وVariance Ratio](hurst_variance_ratio_features.md) | اتجاه الانعكاس/الاستمرار مباشرة | ❌ 12/12 مرفوضة |
| [نسبة التقلّب القفزي (Bipower Variation)](jump_ratio_feature.md) | بنية التقلّب (قفزي مقابل مستمرّ) | ❌ 6/6 مرفوضة |

**36 محاولة إجمالاً، صفر قبول.** هذا يقوّي الاستنتاج الوارد في القسم (١) أعلاه بدليل تجريبي مباشر إضافي: ليس فقط أن "الهندسة المعقّدة" (مذبذبات pandas_ta الكثيرة) لا تتفوّق على البساطة — حتى **البدائل الرياضية الأكفأ نظرياً** لنفس فكرة `NATR_14` (تقيس نفس الشيء بدقّة أعلى، أو تقيس جوانب أعمّ منه كالانعكاس المباشر أو بنية التقلّب) **فشلت في تكرار نجاحه**. الإشارة الحقيقية الوحيدة المؤكَّدة في هذا المشروع بعد ~190 مرشّحاً مُختبَراً تبقى **صيغة `NATR_14` تحديداً** (تنعيم وايلدر، نافذة 14) لا أي معنى أعمّ يمكن تعميمه رياضياً. هذا **يرفع أولوية الاتجاه (٥) أدناه** (تفعيل funding rate/open interest) عملياً، لأنه المسار الوحيد المتبقّي غير المُستنفَد الذي يقدّم **بيانات جديدة كلياً** بدل إعادة صياغة رياضية لنفس بيانات OHLCV المُستنفَدة الآن من كل زاوية معقولة.
